...
def final_tau():
    ...
    report the standard deviation as error. This function both 
    """
    ...
    grouped.to_csv("results/tau_temp.txt", sep="\t", index=False)

    tau_final = pd.read_csv("results/tau_temp.txt", sep='\t')
    ...
    plt.xlim(0.4, 2.6)
    plt.tight_layout()
    plt.savefig('results/tau_temp.pdf')
    plt.show()
